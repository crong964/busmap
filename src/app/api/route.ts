...
export async function GET(request: NextRequest) {
    ...
    const busstopPath = path.resolve(process.cwd(), "src", "bus-stop", `bus_${bus}_${huong}.json`)
    const lnglatPath = path.resolve(process.cwd(), "src", "lnglat", `lnglat_bus_${bus}_${huong}.json`)
